...
const TICKET_PRICE = 10; // R$ 10,00
const DRAW_DATE = new Date(2026, 6, 5, 20, 0); // Month is 0-indexed: 6 = July

// Generate random taken numbers for demo
const TAKEN_NUMBERS = new Set([
...
export default function Home() {
  ...
  const filteredNumbers = useMemo(() => {
    ...
    if (showAvailableOnly) {
      numbers = numbers.filter(n => !TAKEN_NUMBERS.has(n));
    }
    return numbers;
  }, [showAvailableOnly]);
  ...
  const toggleNumber = (num: number) => {
    if (TAKEN_NUMBERS.has(num)) return;
  ...
  const handleRandomPick = () => {
    const available = Array.from({ length: TOTAL_NUMBERS }, (_, i) => i + 1)
      .filter(n => !TAKEN_NUMBERS.has(n) && !selectedNumbers.includes(n));
      ...
      <section className="relative pt-12 pb-24 px-4 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center relative z-10">
              ...
              <div className="flex items-center gap-2 bg-white px-4 py-3 rounded-2xl shadow-sm border border-slate-100">
               ...
               <div className="flex items-center gap-2 bg-white px-4 py-3 rounded-2xl shadow-sm border border-slate-100">
                ...
                <div className="text-left">
                  ...
                  <p className="font-bold text-foreground">JBL GO 4 + Fones</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative group perspective-1000">
             ...
             >
                <div className="absolute -inset-4 bg-gradient-to-tr from-secondary/40 to-primary/40 rounded-[3rem] blur-xl opacity-70 group-hover:opacity-100 transition-opacity duration-500" />
                <img 
                  src={prizeImage} 
                ...
                <AnimatePresence mode="popLayout">
                  {currentNumbers.map((num) => (
                    <RaffleNumber 
                      key={num} 
                      number={num} 
                      status={TAKEN_NUMBERS.has(num) ? "taken" : selectedNumbers.includes(num) ? "selected" : "available"}
          ...
          <div className="bg-accent/20 rounded-3xl p-6 border border-accent/30">
             ...
             <ul className="space-y-3 text-sm text-foreground/80">
               <li className="flex gap-2">
                 <Check className="w-5 h-5 text-accent-foreground flex-shrink-0" />
                 <span>Caixa de Som JBL GO 4 (Original)</span>
               </li>
               <li className="flex gap-2">
                 <Check className="w-5 h-5 text-accent-foreground flex-shrink-0" />
                 <span>Fone Bluetooth Max Sound</span>
               </li>
               <li className="flex gap-2">
                 <Check className="w-5 h-5 text-accent-foreground flex-shrink-0" />
